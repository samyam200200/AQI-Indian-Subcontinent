...
for(i in 1:length(files)) {
  files[i]
  med<- h5read(files[i], "/HDFEOS/GRIDS/OMI Total Column Amount SO2/Data Fields/ColumnAmountSO2")
  med[med<=0]<-NA
  med <- raster(med)
  med<-t(med)
  ...
  dataSO2<-rbind(dataSO2, first)
}

dataSO2$group<-ifelse(grepl("2020", dataSO2$day)==1,2020,2019)

dataSO2$day<-as.Date(dataSO2$day)

dataSO2$onlyC<-format(as.Date(dataSO2$day), "%m-%d")
dataSO2$id<-c(seq(1:184),seq(1:184))
...
library(reshape)
newdataSO2$value[newdataSO2$value=="Inf"]<-0
newdataSO2<-melt(data=dataSO2, id.vars=c("day", "group","id", "onlyC"), measure.vars = c("average", "maximum", "minimum"))

newdataSO2$variable_f = factor(newdataSO2$variable, levels=c('maximum','average','minimum'))
breaks<-c("March 1", "March 20", "April 9", "April 29", "May 19", "June 8", "June 28","July 18", "August 7", "August 27", "August")

ggplot()+
  geom_line(data=newdataSO2, aes(x=id, 
                                 y=value,
                                 group=group, 
                                 col=factor(group)), 
            lwd=1)+
  geom_smooth(data=newdataSO2[newdataSO2$variable=="average",],
              aes(x=id, 
                  y=value, 
                  group=group, 
                  col=factor(group)),
  ...
  ylab(expression(paste("Level of SO"[2]," DU")))+
  scale_x_continuous(n.breaks = 10, 
                     labels = breaks)+
  theme_minimal()+
  theme(text = element_text(size=15), 
...
write.csv(newdataSO2, "newdataSO2.csv")
