...
subtract<- disaggregate(subtract, 3, method='bilinear')
my.at <- seq(-1, +1, 0.1)
newcol<-colorRampPalette(c("#FEFFED","#FBE6D6" , "green","cyan", "blue", "#FE01B1","#fc0000"))
par(mfrow=c(1,3))
mapTheme <- rasterTheme(region=jet.colors(256,alpha = 1))
plt <- levelplot(subtract, margin=FALSE,cuts=100,at=my.at,par.settings=mapTheme,main="SO2 Index - Diffrence July(2020-2019)",shrink=c(1,1), interpolate=T, cex=2)
...
qnorm(.975)*(sd(dataapr2019, na.rm=TRUE)/sqrt(length(dataapr2019)))
