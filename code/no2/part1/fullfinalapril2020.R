...
plt <- levelplot(mean1, margin=FALSE,at=my.at,cuts=100 ,par.settings=mapTheme, main="NO2 Index - April 2020 - Molecules/cm^2",shrink=c(1,1), interpolate=T)
...
qnorm(.975)*(sd(dataapr2020, na.rm=TRUE)/sqrt(202080))
...
qnorm(.975)*(sd(dataapr2019, na.rm=TRUE)/sqrt(202080))
dataapr2019<-data.frame("data"=dataapr2019, "x"=rep("April 2019", times=length(dataapr2019)))
data20202019apr<-rbind(dataapr2019,dataapr2020)
data20202019B<-data.frame(data20202019apr, "group"="B")
...
mapTheme <- rasterTheme(region=jet.colors(256,alpha = 1))
plt <- levelplot(subtract, margin=FALSE,par.settings=mapTheme,at=my.at, cuts=20,main="NO2 Index - Diffrence April(2020-2019)",shrink=c(1,1), interpolate=T)
plt + layer(sp.lines(world_outline, col="black", lwd=1.2))
