...
plt <- levelplot(mean1, margin=FALSE,at=my.at,cuts=100 ,par.settings=mapTheme, main="NO2 Index - July 2020 - Molecules/cm^2",shrink=c(1,1), interpolate=T)
plt + layer(sp.lines(world_outline, col="black", lwd=1))

mean(values(mean1), na.rm=T)
datajul2020<-mean1@data@values
mean(datajul2020, na.rm=TRUE)
sd(datajul2020, na.rm=TRUE)
qnorm(.975)*(sd(datajul2020, na.rm=TRUE)/sqrt(202080))
datajul2020<-data.frame("data"=datajul2020, "x"=rep("July 2020", times=length(datajul2020)))


datajul2019<-mean2@data@values
mean(datajul2019, na.rm=TRUE)
sd(datajul2019, na.rm=TRUE)
qnorm(.975)*(sd(datajul2019, na.rm=TRUE)/sqrt(202080))
datajul2019<-data.frame("data"=datajul2019, "x"=rep("July 2019", times=length(datajul2019)))
data20202019Jul<-rbind(datajul2019,datajul2020)
data20202019B<-data.frame(data20202019Jul, "group"="B")
...
mapTheme <- rasterTheme(region=jet.colors(256,alpha = 1))
plt <- levelplot(subtract, margin=FALSE,par.settings=mapTheme,at=my.at, cuts=20,main="NO2 Index - Diffrence July(2020-2019)",shrink=c(1,1), interpolate=T)
