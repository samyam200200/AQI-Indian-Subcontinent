...
plt <- levelplot(mean1, margin=FALSE,at=my.at,cuts=100 ,par.settings=mapTheme, main="NO2 Index - August 2020 - Molecules/cm^2",shrink=c(1,1), interpolate=T)
plt + layer(sp.lines(world_outline, col="black", lwd=1.2))

dataaug2020<-mean1@data@values
mean(dataaug2020, na.rm=TRUE)
sd(dataaug2020, na.rm=TRUE)
qnorm(.975)*(sd(dataaug2020, na.rm=TRUE)/sqrt(length(dataaug2020)))
dataaug2020<-data.frame("data"=dataaug2020, "x"=rep("August 2020", times=length(dataaug2020)))


dataaug2019<-mean2@data@values
mean(dataaug2019, na.rm=TRUE)
sd(dataaug2019, na.rm=TRUE)
qnorm(.975)*(sd(dataaug2019, na.rm=TRUE)/sqrt(length(dataaug2019)))
dataaug2019<-data.frame("data"=dataaug2019, "x"=rep("August 2019", times=length(dataaug2019)))
data20202019aug<-rbind(dataaug2019,dataaug2020)
data20202019C<-data.frame(data20202019aug, "group"="C")
...
mapTheme <- rasterTheme(region=jet.colors(256,alpha = 1))
plt <- levelplot(subtract, margin=FALSE,par.settings=mapTheme,at=my.at, cuts=20,main="NO2 Index - Diffrence August(2020-2019)",shrink=c(1,1), interpolate=T)
plt + layer(sp.lines(world_outline, col="black", lwd=1.2))
