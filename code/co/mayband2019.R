...
sds <- get_subdatasets("AIRS.2019.05.01.L3.RetStd_IR031.v6.0.31.1.G19218162943.hdf")
View(sds)
gdal_translate(sds[150], dst_dataset = "1502019May.tif")
gdal_translate(sds[328], dst_dataset = "3282019May.tif")
rast <- raster("1502019May.tif", band=6)
rast1 <- raster("3282019May.tif", band=6)
...
plt <- levelplot(diffrence, #dataset
                 ...
                 at=my.at,
                 par.settings=mapTheme, #mapTheme provided previously
                 main="CO Index - May (2020-2019) - 500hpa", #providing the title
                 interpolate=T) #minimal interpolation if there are missing values
plt + layer(sp.lines(world_outline, col="black", lwd=1.0))
