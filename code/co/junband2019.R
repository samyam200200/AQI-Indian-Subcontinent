...
sds <- get_subdatasets("AIRS.2019.06.01.L3.RetStd_IR030.v6.0.31.1.G19184143737.hdf")
View(sds)
gdal_translate(sds[150], dst_dataset = "1502019Jun.tif")
gdal_translate(sds[328], dst_dataset = "3282019Jun.tif")
rast <- raster("1502019Jun.tif", band=6)
rast1 <- raster("3282019Jun.tif", band=6)
...
plt <- levelplot(diffrence, #dataset
                 ...
                 at=my.at,
                 par.settings=mapTheme, #mapTheme provided previously
                 main="CO Index - June (2020-2019) - 500hpa", #providing the title
                 interpolate=T) #minimal interpolation if there are missing values
plt + layer(sp.lines(world_outline, col="black", lwd=1.0))
